...
const renderChildren = (props: ButtonProps) => {
  return (
    <>
      {props.loading && !props.disabled && <ActivityIndicator style={[styles.iconStyle]} color={props.indicatorColor || theme.brandColor[props.color]} />}
      {props.leftIcon || (props.icon && <View style={[styles.iconStyle, props.leftIconStyle]}>{props.leftIcon || props.icon}</View>)}
      <Text style={StyleSheet.flatten([getTextStyle(props), props.textStyle])}>{props.children}</Text>
      {props.rightIcon && <View style={[styles.iconStyle, props.rightIconStyle]}>{props.rightIcon}</View>}
    </>
  );
};
...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <TouchableElement {...props} onPress={props.onPress} disabled={props.disabled || props.loading}>
      <View style={StyleSheet.flatten([getContainerStyle({ ...props }), style])}>{renderChildren({ ...props })}</View>
    </TouchableElement>
  );
};
