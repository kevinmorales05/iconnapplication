...
const EnterPasswordController: React.FC = () => {
  ...
  const onSubmit = async (password: string) => {
    loader.show();
    try {
      const response = await dispatch(signInWithEmailAndPasswordThunk({ email: email!, pass: password }));
      ...
      if (response.error) {
        ...
        if (response.payload) {
          if (!response.payload.additionalUserInfo.isNewUser) {
            const { payload: payloadSignIn } = await dispatch(getUserThunk({ user_id: response.payload.user.uid }));
